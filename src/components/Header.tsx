...
export const Header: React.FC<HeaderProps> = ({
  ...
  const { isAndroid, canInstall, promptInstall } = usePWAInstall();
  ...
  return (
    <header className="h-14 sm:h-16 md:h-18 bg-white/[0.03] backdrop-blur-2xl border-b border-white/10 px-3 sm:px-4 md:px-6 flex items-center justify-between z-30 sticky top-0 select-none">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        {/* Android / PWA Direct Install Button - ONLY displayed when Android device is detected */}
        {isAndroid && (
          <button
            id="header-android-install-btn"
            onClick={handleAndroidDownload}
            className="flex items-center gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-full bg-emerald-500/20 hover:bg-emerald-500/35 border border-emerald-500/40 text-emerald-300 text-xs font-bold transition active:scale-95 cursor-pointer shadow-lg shadow-emerald-500/20 shrink-0"
            title="Télécharger / Installer l'application iSTB sur Android"
          >
            <Smartphone className="w-4 h-4 text-emerald-400 shrink-0" />
            <span className="text-[11px] sm:text-xs font-bold text-emerald-200">
              <span className="hidden sm:inline">Télécharger </span>Android
            </span>
            <Download className="w-3.5 h-3.5 text-emerald-300 shrink-0 ml-0.5" />
          </button>
        )}
